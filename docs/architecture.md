# 基于知识库的 AI 辅助 libFuzzer 驱动生成平台架构文档

## 1. 文档目标

本文档用于指导项目早期工程开发，目标是定义一个基于知识库的 AI 辅助 `libFuzzer` 驱动生成平台的高层架构。

本文中的 Fuzz 驱动生成特指面向 `libFuzzer` 的驱动生成，即生成符合如下入口形式的 C/C++ fuzz driver：
...
- `PromeFuzz: A Knowledge-Driven Approach to Fuzzing Harness Generation with Large Language Models`：强调知识库、API 语义、代码元数据、文档知识、API 相关性和运行反馈对驱动生成质量的提升。

本平台的核心思想是：将目标库相关知识沉淀到知识库中，再由多个面向 fuzzing 生命周期的 Agent 使用这些知识完成驱动生成、驱动执行、种子生成、Crash 分析和反馈迭代。

## 3. 平台目标与边界

### 3.1 平台目标

平台目标是围绕 C/C++ 库构建一个 AI 辅助的 `libFuzzer` 驱动生成与执行闭环：

1. 从目标库中提取 API、类型、文档、示例和历史执行反馈等知识。
2. 基于知识库生成符合 `libFuzzer` 规范的 fuzz driver。
3. 基于知识库生成或优化 Fuzz 种子输入。
4. 执行 fuzz driver，收集 Crash、覆盖率、运行日志和新种子。
5. 分析 Crash，定位问题，并区分真实库缺陷与驱动误用。
...
- 目标对象：C/C++ 库。
- Fuzz 引擎：`libFuzzer`。
- 驱动入口：`LLVMFuzzerTestOneInput`。
- 核心目标：跑通“知识库 -> 驱动生成 -> 种子生成 -> 执行 -> Crash/覆盖率分析 -> 反馈”的最小闭环。
...
2. 调度组件根据当前状态选择下一步任务。
3. Fuzz 驱动生成 Agent 使用知识库和大模型生成 `libFuzzer` 驱动。
4. Fuzz 驱动执行 Agent 使用知识库和大模型生成更好的 Fuzz 种子，并执行驱动。
...
- 反馈知识：编译失败原因、驱动误用、Crash 结论、覆盖率瓶颈。
...
这些能力优先服务于 Fuzz 驱动生成 Agent，帮助其构造 API 调用上下文；也服务于 Fuzz 驱动执行 Agent 和 Crash 分析 Agent，用于推断种子约束、分析调用路径和定位驱动误用。

初始阶段可以先采用简单存储形式，例如文件目录、JSON、SQLite 或轻量索引；后续再根据规模演进为图数据库、向量数据库和结构化数据库组合。

### 5.2 Fuzz 驱动生成 Agent

Fuzz 驱动生成 Agent 负责根据知识库输入，与大模型交互生成 `libFuzzer` 驱动。

主要职责：

- 选择或接收目标 API。
- 从知识库检索 API 声明、类型依赖、调用序列、文档约束和历史反馈。
- 生成 `LLVMFuzzerTestOneInput` 驱动代码。
- 根据编译错误和运行早期反馈修复驱动。
- 将成功和失败经验回写知识库。
...
初始阶段只需要支持单文件驱动生成和基础编译修复；后续再扩展到多 API 组合、复杂对象构造、状态机路径和多驱动合并。

### 5.3 Fuzz 驱动执行 Agent

Fuzz 驱动执行 Agent 负责组织 `libFuzzer` 驱动执行，并根据知识库输入通过大模型生成或优化 Fuzz 种子。

主要职责：

- 编译和运行生成的 `libFuzzer` 驱动。
...
- 结合知识库中的 API 约束、源码片段、驱动代码和历史反馈分析 Crash。
- 判断 Crash 更可能是真实库缺陷、驱动误用、环境问题还是低价值异常。
...
1. 构建知识库。
2. 选择一个或一组目标 API。
3. 调用 Fuzz 驱动生成 Agent。
4. 调用 Fuzz 驱动执行 Agent。
5. 调用 Crash 分析 Agent。
6. 回写知识库。
7. 如果预算允许，进入下一轮。
...
stateDiagram-v2
    [*] --> BuildKnowledge
    BuildKnowledge --> GenerateDriver
    GenerateDriver --> ExecuteDriver
    ExecuteDriver --> AnalyzeCrashAndCoverage
    AnalyzeCrashAndCoverage --> UpdateKnowledge
    UpdateKnowledge --> ScheduleNext
    ScheduleNext --> GenerateDriver: continue
    ScheduleNext --> [*]: stop
```
...
- 知识库为驱动生成提供上下文。
- 驱动生成 Agent 产出 `libFuzzer` 驱动。
- 驱动执行 Agent 生成种子并执行 fuzzing。
...
- 导入目标库源码。
- 抽取基础 API 信息。
- 生成一个 `LLVMFuzzerTestOneInput` 驱动。
- 生成一批初始种子。
- 执行 `libFuzzer`。
- 收集 Crash 和基础覆盖率。
- 将结果写回知识库。
...
3. 能生成至少一个 `LLVMFuzzerTestOneInput` 驱动。
4. 能基于知识库生成或准备初始种子。
5. 能执行 `libFuzzer` 并保存运行日志。
6. 能收集 Crash 输入和基础覆盖率。
7. 能输出 Crash 初步分析结论。
8. 能将驱动、种子、执行结果和分析结论回写知识库。
